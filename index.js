...
function get20s(artists) {
  ...
  for (let i = 0; i < artists.length; i++) {
    // Convert years field to 2 numbers
    yearStart = artists[i].years.substr(0, 4)
    yearEnd = artists[i].years.substr(7, 4)

    //check for between 1900 - 2000
    if ((yearEnd > 1900 && yearEnd < 2000) || (yearStart > 1900 && yearStart < 2000)) {
      lived20c.push(artists[i].name);
    }
  }
  return lived20c
}
...
const artistAndy = {
  ...
  genre: "Web Design",
  nationality: "USA",
  bio: "Andy is a photographer."
}

addArtist(artistAndy);
